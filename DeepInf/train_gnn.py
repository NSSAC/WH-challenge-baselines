#!/usr/bin/env python
# encoding: utf-8
# File Name: train_gnn.py
# Adapted from train.py by Jiezhong Qiu in https://github.com/xptree/DeepInf/
# Adaptation done by Galen Harrison, 2022-08-11
...
from DeepInfSrc.gcn import BatchGCN
from DeepInfSrc.gat import BatchGAT
from sklearn.metrics import precision_recall_fscore_support
from sklearn.metrics import roc_auc_score
from sklearn.metrics import precision_recall_curve
from sklearn.metrics import average_precision_score
from DeepInfSrc.data_loader import ChunkSampler
from DeepInfSrc.data_loader import InfluenceDataSet
...
parser.add_argument("--model-save-file", default="./deepinf_model.tf")
...
train_loader = DataLoader(influence_dataset, batch_size=args.batch,
                        sampler=ChunkSampler(valid_start - train_start, train_start))
...
def evaluate(epoch, loader, thr=None, return_best_thr=False, log_desc='valid_'):
    ...
    auprc = average_precision_score(y_true, np.exp(y_score))
    torch.save(model.state_dict(), f"{args.model_save_file}_{epoch}.pt")
    logger.info("%sloss: %.4f AUC: %.4f Prec: %.4f Rec: %.4f F1: %.4f AUPRC: %.4f",
            log_desc, loss / total, auc, prec, rec, f1, auprc)
